...
class PreProcessor():
    ...
    def warp_perspect(self, img):
        width_margin = 215
        src = np.float32([[0, 0], [0, self.roi_height-50 ],  [self.roi_width, 0], [self.roi_width, self.roi_height-50 ]]) # Perspective transform을 위한 src 좌표 설정
        dst = np.float32([[0, 0], [width_margin, self.roi_height-50 ], [self.roi_width, 0], [self.roi_width-width_margin, self.roi_height-50 ]]) # Perspective transform을 위한 dst 좌표 설정
        ...
        warped_img = cv2.warpPerspective(roi, M, (self.roi_width, self.roi_height-50), flags=cv2.INTER_LINEAR) # 이미지 워핑으로 Bird Eye View 생성
    ...
    def hist_line_peak(self, img):
        ...
        left_hist_result = np.argmax(histogram[:midpoint-hist_find_margin]) # 중앙점을 기준으로 히스토그램을 계산해 왼쪽 라인 시작점 구함
        right_hist_result = midpoint+hist_find_margin + np.argmax(histogram[midpoint+hist_find_margin:]) # 중앙점을 기준으로 히스토그램을 계산해 오른쪽 라인 시작점 구함
        mid_hist_result = midpoint-mid_hist_find_margin + np.argmax(histogram[midpoint-mid_hist_find_margin:midpoint+mid_hist_find_margin]) # 중앙점을 기준으로 히스토그램을 계산해 오른쪽 라인 시작점 구함
    ...
    def sliding_window(self, img):
        ...
        left_base, mid_base, right_base = self.hist_line_peak(img) # hist_line_peak 함수로 슬라이딩 윈도우의 초기 탐색점 결정
        ...
        while y>0: # window가 이미지 상단에 도달할때까지 반복
            if self.left_window_n < 20: # 왼쪽 차선 검출을 위한 window를 5개까지만 허용 
                window = img[y-self.window_height:y, left_base-self.window_width:left_base+self.window_width] # left window 생성
                ...
                if(len(contours) == 0): # 차선을 못 찾았을떄
                    if prev_detect_flag_left == False: # 차선을 연속으로 못 찾았을떄
                        ...
                        cv2.circle(msk, (left_base-self.window_width-20, y-(self.window_height//2)), 1, (0,50,150), 1)

                    prev_detect_flag_left = False # 직전 차선 검출 성공 플래그 False로 세팅
                    cv2.circle(msk, (left_base-self.window_width, y-(self.window_height//2)), 1, (0,125,125), 1)
                
                if line_detect_fail_count_left < 5: # 차선 검출 실패 카운트가 5 이하일떄 차선으로 판단
                    
                    cv2.circle(msk, (left_base-self.window_width, y-(self.window_height//2)), 1, (0,255,0), 1)
                    ...
                    for contour in contours: 
                        ...
                        if M["m00"] != 0:
                            ...
                            cv2.circle(msk, (left_base-self.window_width + cx, y-(self.window_height//2)), 1, (255,0,0), 1)
                            left_base = left_base-self.window_width + cx # 다음 차선(위쪽 존재) 검출 윈도우 좌표를 현재 검출된 contour 모멘트를 고려해서 수정 
            ...
            if self.mid_window_n < 20: # 왼쪽 차선 검출을 위한 window를 5개까지만 허용 
                window = img[y-self.window_height:y, mid_base-self.window_width:mid_base+self.window_width] # mid window 생성
                contours, _ = cv2.findContours(window, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # window 영역에서 contour 검출

                if(len(contours) == 0): # 차선을 못 찾았을떄
                    if prev_detect_flag_mid == False: # 차선을 연속으로 못 찾았을떄
                        line_detect_fail_count_mid+=1 # 차선 검출 실패 카운트 증가
                        cv2.circle(msk, (mid_base-self.window_width-20, y-(self.window_height//2)), 1, (0,50,150), 1)

                    prev_detect_flag_mid = False # 직전 차선 검출 성공 플래그 False로 세팅
                    cv2.circle(msk, (mid_base-self.window_width, y-(self.window_height//2)), 1, (0,125,125), 1)
                
                if line_detect_fail_count_mid < 3: # 차선 검출 실패 카운트가 5 이하일떄 차선으로 판단
                    cv2.circle(msk, (mid_base-self.window_width, y-(self.window_height//2)), 1, (0,255,0), 1)
                    ...
                    for contour in contours: 
                        ...
                        if M["m00"] != 0:
                            ...
                            cv2.circle(msk, (mid_base-self.window_width + cx, y-(self.window_height//2)), 1, (255,0,0), 1)
                            mid_base = mid_base-self.window_width + cx # 다음 차선(위쪽 존재) 검출 윈도우 좌표를 현재 검출된 contour 모멘트를 고려해서 수정 
            ...
            if self.right_window_n < 20: # 오른쪽 차선 검출을 위한 window를 5개까지만 허용 
                window = img[y-self.window_height:y, right_base-self.window_width:right_base+self.window_width] # right window 생성

                contours, _ = cv2.findContours(window, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # window 영역에서 contour 검출
                if(len(contours) == 0): # 차선을 못 찾았을떄
                    if prev_detect_flag_right == False: # 차선을 연속으로 못 찾았을떄
                        line_detect_fail_count_right+=1 # 차선 검출 실패 카운트 증가
                        cv2.circle(msk, (right_base-self.window_width-20, y-(self.window_height//2)), 1, (0,255,0), 1)

                    prev_detect_flag_right = False # 직전 차선 검출 성공 플래그 False로 세팅
                    cv2.circle(msk, (right_base-self.window_width-10, y-(self.window_height//2)), 1, (0,255,255), 1)
                
                if line_detect_fail_count_right < 5: # 차선 검출 실패 카운트가 5 이하일떄 차선으로 판단
                    cv2.circle(msk, (right_base-self.window_width, y-(self.window_height//2)), 1, (0,0,255), 1)
                    ...
                    for contour in contours:
                        ...
                        if M["m00"] != 0:
                            ...
                            cv2.circle(msk, (right_base-self.window_width + cx, y-(self.window_height//2)), 1, (0,255,0), 1)
                            right_base = right_base-self.window_width + cx  # 다음 차선(위쪽 존재) 검출 윈도우 좌표를 현재 검출된 contour 모멘트를 고려해서 수정 
    ...
    def drawing_lane(self, msk, lx, ly, mx, my, rx, ry):
        if lx != None:
            for i in range(len(lx)):
                cv2.rectangle(msk, (lx[i]-self.window_width,ly[i]), (lx[i]+self.window_width,ly[i]-self.window_height), (255,0,0), 1)
        if rx != None:
            for k in range(len(rx)):
                cv2.rectangle(msk, (rx[k]-self.window_width,ry[k]), (rx[k]+self.window_width,ry[k]-self.window_height), (0,255,0), 1)
        if mx != None:
            for j in range(len(mx)):
                cv2.rectangle(msk, (mx[j]-self.window_width,my[j]), (mx[j]+self.window_width,my[j]-self.window_height), (0,0,255), 1)
    #=============================================
    # 검출된 차선을 화면에 표시
