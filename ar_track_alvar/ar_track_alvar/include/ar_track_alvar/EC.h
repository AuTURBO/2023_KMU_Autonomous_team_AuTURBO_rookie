 ...
 *
 * You can inherit your own classes/structures from this or make one with
 * similar members.
 *
 * The idea is that the EC-versions store the data externally in a map of
 * features that are based on the structure \e ExternalContainer . The \e
 * ExternalContainer has certain predefined fields that are used/needed by
 * different methods. Below is a summary of some main methods for reading (r)
 * and writing (w) in the external container.
 *
 *  <table border>
 *  <tr>
 *     <td></td>
...
public:
  ...
  CvPoint2D32f p2d;
  CvPoint3D32f p3d;
  CvPoint2D32f projected_p2d;  // This is only temporary -- user needs to take
                               // care that it has valid content
...
/** \brief This is a default functor for testing which items in the container
 * should be handled by each method
 *
 *  By default this only tests that the only the \e type_id. If you specify \e
 * type_id that is not -1 then the functor tests that the item's \e type_id
 * matches. Naturally you can make whatever tests in your own functors.
 */
template <typename T>
class DoHandleTest
{
...
public:
  DoHandleTest(int _type_id = -1, bool _needs_has_p2d = false,
               bool _needs_has_p3d = false)
    : type_id(_type_id)
    , needs_has_p2d(_needs_has_p2d)
    , needs_has_p3d(_needs_has_p3d)
  {
  }
  virtual bool operator()(const T& f) const
  {
    if (needs_has_p2d && !f.has_p2d)
      return false;
    if (needs_has_p3d && !f.has_p3d)
      return false;
    // if (f.type_id == -1) return false; // Unnecessary?
    if ((type_id != -1) && (type_id != f.type_id))
      return false;
    return true;
  }
};

/** \brief This is default functor for testing which items in the container
 * should be erased
 *
 *  By default this can test for \e has_p2d and \e has_p3d . Furthermore, it can
 * test for reprojection error when \e Pose , \e Camera and limit are given.
 */
template <typename T>
class DoEraseTest
{
...
public:
  DoEraseTest(bool _erase_without_p2d, bool _erase_without_p3d,
              int _type_id = -1)
    : type_id(_type_id)
    , erase_without_p2d(_erase_without_p2d)
    , erase_without_p3d(_erase_without_p3d)
    , test_reprojection(false)
    , limit_sq(0.f)
  {
  }
  DoEraseTest(float _limit, bool _erase_without_p2d = false,
              bool _erase_without_p3d = false, int _type_id = -1)
    : type_id(_type_id)
    , erase_without_p2d(_erase_without_p2d)
    , erase_without_p3d(_erase_without_p3d)
    , test_reprojection(true)
    , limit_sq(_limit * _limit)
  {
  }
  virtual bool operator()(const T& f) const
  {
    if ((type_id != -1) && (type_id != f.type_id))
      return false;
    if (erase_without_p2d && !f.has_p2d)
      return true;
    if (erase_without_p3d && !f.has_p3d)
      return true;
    if (test_reprojection)
    {
      ...
      double dist_sq =
          (f.p2d.x - f.projected_p2d.x) * (f.p2d.x - f.projected_p2d.x);
...
public:
  /** \brief Constructor */
  TrackerFeaturesEC(int _max_features = 100, int _min_features = 90,
                    double _quality_level = 0.01, double _min_distance = 10,
                    int _pyr_levels = 4, int win_size = 6)
    : TrackerFeatures(_max_features, _min_features, _quality_level,
                      _min_distance, _pyr_levels, win_size)
    , purge(false)
  {
  }

  /** \brief Track features with matching type id. New features will have id's
   * in the specified id range. */
  template <typename T>
  bool Track(IplImage* img, IplImage* mask, std::map<int, T>& container,
             int type_id = -1, int first_id = -1, int last_id = -1)
  {
    DoHandleTest<T> do_handle_test_default(type_id);
    if (type_id == -1)
      type_id = 0;
    return Track(img, mask, container, do_handle_test_default, type_id,
                 first_id, last_id);
  }
   ...
   * min_features */
  template <typename T>
  bool AddFeatures(std::map<int, T>& container, int type_id = 0,
                   int first_id = 0, int last_id = 65535)
  {
  ...
  /** \brief Erases the items matching with \e type_id having \e has_p2d ==
   * false . If \e type_id == -1 doesn't test the type. */
  template <typename T>
  ...
  /** \brief Purge features that are considerably closer than the defined
   * min_distance.
   *
   * Note, that we always try to maintain the smaller id's assuming that they
   * are older ones
   */
  void Purge()
  {
  ...
  void Reset()
  {
    throw alvar::AlvarException("Method not supported");
  }
...
public:
  ...
  template <typename T>
  bool CalcExteriorOrientation(std::map<int, T>& container, Pose* pose,
                               int type_id = -1)
  {
    DoHandleTest<T> do_handle_test_default(type_id);
                               ...
                               F do_handle_test)
  {
    ...
    bool ret = Camera::CalcExteriorOrientation(&world_points_sub,
                                               &image_observations_sub, pose);
  ...
  template <typename T>
  bool UpdatePose(std::map<int, T>& container, Pose* pose, int type_id = -1,
                  std::map<int, double>* weights = 0)
  {
    DoHandleTest<T> do_handle_test_default(type_id);
    return UpdatePose(container, pose, do_handle_test_default, weights);
  }
  /** \brief Update the rotation in \e pose using the items with matching \e
   * type_id */
  template <typename T>
  ...
  /** \brief Update the rotation in \e pose using the items  matching the given
   * functor */
  template <typename T, typename F>
  bool UpdateRotation(std::map<int, T>& container, Pose* pose, F do_handle_test)
  {
    ...
    CvMat* image_observations =
        cvCreateMat(count_points * 2, 1, CV_64F);  // [u v u v u v ...]'
  ...
  template <typename T, typename F>
  bool UpdatePose(std::map<int, T>& container, Pose* pose, F do_handle_test,
                  std::map<int, double>* weights = 0)
  {
    int count_points = container.size();
    if (count_points < 4)
      return false;  // Note, UpdatePose calls CalcExteriorOrientation if 4 or 5
                     // points
    ...
    for (; iter != iter_end; iter++)
    {
      ...
      if (f.has_p2d && f.has_p3d)
      {
        ...
        if (weights)
          w->data.db[ind * 2 + 1] = w->data.db[ind * 2 + 0] =
              (*weights)[iter->first];
        ind++;
      }
    }
    ...
    if (weights)
    {
      ...
      ret =
          UpdatePose(&world_points_sub, &image_observations_sub, pose, &w_sub);
    }
    else
  ...
  /** \brief Projects \e p3d in the items matching with \e type_id into \e
   * projected_p2d . If \e type_id == -1 doesn't test the type. */
  template <typename T>
  float Reproject(std::map<int, T>& container, Pose* pose, int type_id = -1,
                  bool needs_has_p2d = false, bool needs_has_p3d = false,
  ...
  template <typename T, typename F>
  float Reproject(std::map<int, T>& container, Pose* pose, F do_handle_test,
                  float average_outlier_limit = 0.f)
  {
    ...
    for (; iter != iter_end; iter++)
    {
      ...
      Camera::ProjectPoint(iter->second.p3d_sh, pose,
                           iter->second.projected_p2d_sh);

      // TODO: Now this is calculated in several places (should this distance be
      // included in ExternalContainer structure?
      float dist_sq =
          (f.p2d.x - f.projected_p2d.x) * (f.p2d.x - f.projected_p2d.x);
      dist_sq += (f.p2d.y - f.projected_p2d.y) * (f.p2d.y - f.projected_p2d.y);
  ...
  /** \brief Erases the items matching with \e type_id having large reprojection
   * error. If \e type_id == -1 doesn't test the type. If \e pose is given calls
   * \e Reproject() internally -- otherwise assumes it has been called
   * beforehands.
   */
  template <typename T>
  int EraseUsingReprojectionError(std::map<int, T>& container,
                                  float reprojection_error_limit,
                                  int type_id = -1, Pose* pose = 0)
  {
    if (pose)
      Reproject(container, pose, type_id, false, false);
    DoEraseTest<T> do_erase_test(reprojection_error_limit, false, false,
                                 type_id);
    return EraseItemsEC(container, do_erase_test);
  }

  /** \brief Update pose rotations based on new observations */
  bool UpdateRotation(const CvMat* object_points, CvMat* image_points,
                      Pose* pose);
  ...
  bool UpdatePose(const CvMat* object_points, CvMat* image_points, Pose* pose,
                  CvMat* weights = 0);

  /** \brief Update existing pose (in rodriques&tra) based on new observations
   */
  bool UpdatePose(const CvMat* object_points, CvMat* image_points,
                  CvMat* rodriques, CvMat* tra, CvMat* weights = 0);

  /** \brief Reconstruct 3D point using two camera poses and corresponding
   * undistorted image feature positions */
  bool ReconstructFeature(const Pose* pose1, const Pose* pose2,
                          const CvPoint2D32f* u1, const CvPoint2D32f* u2,
                          CvPoint3D32f* p3d, double limit);

  /** \brief Get 3D-coordinate for 2D feature on the plane defined by the pose
   * (z == 0) */
  ...
  void Get3dOnDepth(const Pose* pose, CvPoint2D32f p2d, float depth,
                    CvPoint3D32f& p3d);
};

/** \brief Calculate the index used in external container map for specified \e
 * marker_id */
inline int MarkerIdToContainerId(int marker_id, int corner_id, int first_id = 0,
                                 int last_id = 65535)
{
...
public:
  ...
  /** \brief Detect markers in the image and fill in their 2D-positions in the
   * given external container */
  template <typename T>
  int Detect(IplImage* image, Camera* cam, std::map<int, T>& container,
             int type_id = 0, int first_id = 0, int last_id = 65535,
             bool track = false, bool visualize = false,
             double max_new_marker_error = 0.08, double max_track_error = 0.2,
             LabelingMethod labeling_method = CVSEQ)
  {
    int ret;

    // The existing markers in the container are marked to not have valid p2d
    // (has_p2d=false)
    typename std::map<int, T>::iterator iter = container.begin();
    typename std::map<int, T>::iterator iter_end = container.end();
    for (; iter != iter_end; iter++)
    ...
    ret = MarkerDetector<M>::Detect(image, cam, track, visualize,
                                    max_new_marker_error, max_track_error,
                                    labeling_method, false);
  ...
  /** \brief Fill in 3D-coordinates for \e marker_id marker corners using \e
   * edge_length and \e pose */
  template <typename T>
  void MarkerToEC(std::map<int, T>& container, int marker_id,
                  double edge_length, Pose& pose, int type_id = 0,
                  int first_id = 0, int last_id = 65535)
  {
    CvMat* m3 = cvCreateMat(4, 4, CV_64F);
    cvSetIdentity(m3);
    pose.GetMatrix(m3);
...
public:
  ...
  template <typename T>
  bool MarkersToEC(std::map<int, T>& container, int type_id = 0,
                   int first_id = 0, int last_id = 65535)
  {
    bool ret = false;
    for (size_t i = 0; i < marker_indices.size(); i++)
    {
      if (marker_status[i] == 0)
        continue;  // Skip the ones not in point cloud
  ...
  template <typename T>
  bool MarkersFromEC(std::map<int, T>& container, int type_id = 0,
                     int first_id = 0, int last_id = 65535)
  {
    // TODO...
    return false;
  ...
  template <typename T>
  bool Save(std::map<int, T>& container, const char* fname,
            FILE_FORMAT format = FILE_FORMAT_DEFAULT, int type_id = 0,
            int first_id = 0, int last_id = 65535)
  {
    if (!MarkersFromEC(container, type_id, first_id, last_id))
      return false;
    if (!MultiMarker::Save(fname, format))
      return false;
    return true;
  }

  /** \brief Fill in 3D-coordinates for marker corners to the given container
   * using save \e MultiMarker file */
  template <typename T>
  bool Load(std::map<int, T>& container, const char* fname,
            FILE_FORMAT format = FILE_FORMAT_DEFAULT, int type_id = 0,
            int first_id = 0, int last_id = 65535)
  {
    if (!MultiMarker::Load(fname, format))
      return false;
