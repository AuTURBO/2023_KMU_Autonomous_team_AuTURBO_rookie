	...
	# LEFT VERSION
        if PART == 2:
            if (ob.mode == Position.LEFT):
                obstacle_cnt += 1
		if obstacle_cnt == 1:   
                    for theta in range(270, 500, 14): #10  # (270, 515, 16)
                        st = 0.6 * np.sin(theta * np.pi / 180) #0.68
                        drive(st, car_run_speed)
                        time.sleep(0.1)

		elif obstacle_cnt >= 3:
		    for theta in range(315, 470, 10):   # (270, 515, 16)
                ...
                obstacle_cnt += 1
                if obstacle_cnt == 2:
                    for theta in range(300, 530, 15): # (280, 440, 10) #270,450, 8
                        st = 0.8 * np.sin(theta * np.pi / 180) #0.9
                        drive(-st, car_run_speed)
                        time.sleep(0.1)
            
#for theta in range(270, 500, 8): # (280, 440, 10) #270,450, 8
	...
	cw_list.append(slidingwindow.cw_sum)
       ...
       # if slidingwindow.cw_sum >= 2900 and cw_sum_old >= 2900: # check!!!!!!!!!!!!
        if PART == 3:
            ...
            if sum(cw_list) // 4 >= 1975:#3020: #0925 jajus 3020:  # if stop_detected: 210
                drive(0, 0)
                time.sleep(8)
                crosswalk_cnt += 1
		...
		continue
	if PART == 4:
	    if (sum(cw_list)//4 >= 2800) and ((curve_detector.curve_count - part_3_curve_count) >= 1): #0925_jajus 3350 #3200
		PART = 1
                obstacle_cnt = 0
                curve_detector.curve_count = 0
		stop_cnt+=1
