...
def start():
	...
	# RIGHT VERSION
		...
		obstacle_cnt += 1
		if obstacle_cnt ==1 :
                    for theta in range(280, 360, 10): #270,450, 8
                        st = 0.72 * np.sin(theta * np.pi / 180) #0.9
                        drive(-st, car_run_speed)
                        time.sleep(0.1)
		if obstacle_cnt == 3:
 		    for theta in range(270, 480, 11): #270,450, 8
                        st = 0.72 * np.sin(theta * np.pi / 180) #0.9
                        drive(-st, car_run_speed)
                        time.sleep(0.1)
                continue
...
#        stop_detected = stop_counter.check_stop_line(image)
	...
	if PART == 4:
	    if sum(cw_list)//4 >= 3200 and (curve_detector.curve_count - part_3_curve_count) >= 1:
		PART = 1
                obstacle_cnt = 0
                curve_detector.curve_count = 0
		stop_cnt += 1
