...
def process_image(frame):
    ...
    # low_threshold = 45
    # high_threshold = 100
    low_threshold = 35 #0925 jajus 60 # 2 floor : 35  # 41
...
def start():
	...
	# LEFT VERSION
		...
		if obstacle_cnt == 1:   
		    ...
		    for theta in range(315, 465, 10):   # (270, 515, 16)
	...
	cw_list.append(slidingwindow.cw_sum)
       ...
       # if slidingwindow.cw_sum >= 2900 and cw_sum_old >= 2900: # check!!!!!!!!!!!!
        if PART == 3:
            ...
            if sum(cw_list) // 4 >= 1900: #0925 jajus 3020:  # if stop_detected: 210
                drive(0, 0)
                time.sleep(8)
                crosswalk_cnt += 1
		for i in range(12):
		...
		continue
	if PART==4:
	    if sum(cw_list)//4 >= 2730: #0925_jajus 3350
		PART = 1
                obstacle_cnt = 0
                curve_detector.curve_count = 0
